...
fetch_latest_version__173_v0() {
    ...
    local command_8
    command_8="$(bash -o pipefail -c "(wget -S --max-redirect=0 --spider '${url_50}' 2>&1 || true) | sed -n \"s#^[[:space:]]*[Ll]ocation:[[:space:]]*.*/##p\" | sed \"s#[[:space:]]*\[following\]##\" | tail -n1 | grep . || (echo \"Unable to determine latest release version from ${url_50}\" >&2; exit 1)")"
    __status=$?
    if [ "${__status}" != 0 ]; then
