...
filename_74="${__BINARY_NAME_3}-${version_41}-${arch_73}-unknown-linux-musl.tar.gz"
release_url_75="https://github.com/${__REPO_OWNER_4}/${__REPO_NAME_5}/releases/download/${version_41}"
...
extract_archive__180_v0 "${filename_74}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
...
# Clean up
cleanup_temp_dir__182_v0 "${temp_dir_70}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
