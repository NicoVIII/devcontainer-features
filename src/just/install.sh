...
# Config
__BINARY_NAME_3="just"
__REPO_OWNER_4="casey"
__REPO_NAME_5="just"
...
filename_14="${__BINARY_NAME_3}-${version_8}-${arch_13}-unknown-linux-musl.tar.gz"
release_url_15="https://github.com/${__REPO_OWNER_4}/${__REPO_NAME_5}/releases/download/${version_8}"
url_16="${release_url_15}/${filename_14}"
echo "Installing ${__BINARY_NAME_3} ${version_8}..."
download_file__136_v0 "${url_16}" "${filename_14}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
extract_archive__141_v0 "${filename_14}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
...
# Clean up
cleanup_temp_dir__143_v0 "${temp_dir_11}"
__status=$?
if [ "${__status}" != 0 ]; then
    exit "${__status}"
fi
